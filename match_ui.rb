...
class MatchUI
  ...
  def initialize
    ...
    @board_font = Gosu::Font.new G.window, 'data/font/DroidSans-Bold.ttf', BoardFontSize
  end

  def update
    @over = true if Controller.key_pressed?(Gosu::KbSpace)

    if Controller.key_pressed?(Gosu::MsLeft) and mouse_on_board?
    ...
    if @waiting_number
      ...
      if number > -1
        ...
        @match.board[@selected_y][@selected_x] = number
        @selected_x = -1
        @selected_y = -1
      end
    end
